-- Manual fix for the "brand_id is ambiguous" error in the record_price_change() function
...
BEGIN
  ...
  IF NEW.product_id IS NULL THEN
    ...
    IF matched_product_id IS NULL AND NEW.brand IS NOT NULL AND NEW.sku IS NOT NULL AND NEW.brand != '' AND NEW.sku != '' THEN
      ...
      SELECT id INTO v_brand_id FROM brands WHERE user_id = NEW.user_id AND name = NEW.brand LIMIT 1;

      -- If brand found, try to match by brand_id and SKU
      -- Fixed: Use v_brand_id instead of brand_id to avoid ambiguity
...
COMMENT ON FUNCTION public.record_price_change() IS 'Processes scraped products to match them with existing products and record price changes. Fixed to avoid brand_id ambiguity.';
