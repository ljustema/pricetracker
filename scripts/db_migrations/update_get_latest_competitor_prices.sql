-- Migration to update the get_latest_competitor_prices function to handle the new url column
...
-- Create the updated function with the url column included
...
BEGIN
    ...
    WITH LatestCompetitorPrices AS (
        SELECT
            ...
            NULL::TEXT AS url, -- Add url column (null for competitor prices)
    ...
    LatestIntegrationPrices AS (
        SELECT
            ...
            p.url AS url, -- Get url from products table
    ...
    ORDER BY changed_at DESC; -- This is fine because the column name is unambiguous in the result set
END;
$$ LANGUAGE plpgsql;

-- Also update the get_product_price_history function to include the url column
...
SELECT 'Migration completed: Updated get_latest_competitor_prices and get_product_price_history functions to handle the url column.' AS result;
