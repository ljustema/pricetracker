...
const CONTEXT = 'API:refresh-prices-status';

/**
 * GET /api/products/refresh-prices-status
 *
 * Gets the current status of the materialized view refresh operation.
 *
 * Returns:
...
export async function GET(_request: NextRequest) {
  try {
    ...
    const { data, error } = await supabase
      ...
      .select('*')
      .eq('view_name', 'latest_product_data_mv')
      .single();
