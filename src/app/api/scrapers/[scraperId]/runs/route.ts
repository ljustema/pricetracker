...
import { ensureUUID } from '@/lib/utils/uuid'; // Assuming ensureUUID is moved to a utils file

export async function GET(
  request: NextRequest,
  { params }: { params: { scraperId: string } } // Destructure scraperId directly
) {
  try {
    ...
    if (!supabaseUrl || !supabaseServiceRoleKey) {
      console.error("Server configuration error: Supabase URL or Service Role Key missing");
      return NextResponse.json({ error: "Server configuration error" }, { status: 500 });
    }

    const supabase = createClient(supabaseUrl, supabaseServiceRoleKey);

    // 1. Verify the user owns the scraper
    ...
    if (runsError) {
      ...
      throw new Error(`Failed to fetch scraper runs: ${runsError.message}`);
    }

    return NextResponse.json(runs || []); // Return empty array if runs is null/undefined
