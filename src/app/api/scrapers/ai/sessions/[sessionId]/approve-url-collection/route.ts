...
// POST handler to approve the URL collection phase
export async function POST(request: NextRequest, { params }: Params) {
  // Extract sessionId from params - using await for Next.js 15 compatibility
  const { sessionId } = await params;
  const routeContext = `API:scrapers/ai/sessions/${sessionId}/approve-url-collection`;
  logger.info(routeContext, `Received request to approve URL collection for session: ${sessionId}`);
  ...
  try {
    ...
    // 5. Approve the URL collection data and advance to the next phase
    logger.info(routeContext, `Approving URL collection data for session: ${sessionId}`);
    const updatedSession = await ScraperSessionService.approveUrlCollectionData(
      sessionId,
      body.userFeedback
    );

    // 6. Return the updated session
    logger.info(routeContext, `URL collection approved successfully for session: ${sessionId}`);
    return NextResponse.json(updatedSession);
  } catch (error) {
    logger.error(routeContext, `Error approving URL collection: ${error instanceof Error ? error.message : "Unknown error"}`, error);
    return NextResponse.json(
