...
def process_job(conn, job):
    ...
    try:
        ...
        if filter_by_active_brands:
            try:
                with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
                    cur.execute("SELECT name FROM brands WHERE user_id = %s AND is_active = TRUE;", (user_id,))
                    brands = cur.fetchall()
        ...
        if scrape_only_own_products:
            try:
                with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
                    cur.execute("SELECT ean, sku, brand FROM products WHERE user_id = %s AND is_active = TRUE;", (user_id,))
                    products = cur.fetchall()
                    own_product_eans = [row['ean'] for row in products if row['ean']]
                    own_product_sku_brands = [{'sku': row['sku'], 'brand': row['brand']} for row in products if row['sku'] and row['brand']]
