...
// TODO: Generate Supabase types (e.g., using `supabase gen types typescript`) and import the `Database` type
...
// Using `any` for now as Database types are not available. Generate types for better safety.
...
async function fetchAndProcessJob() {
  ...
  try {
    ...
    let ownProductSkuBrands: { sku: string; brand: string }[] = [];
    ...
    if (scraper.scrape_only_own_products) {
        ...
        if (productError) {
            ...
            ownProductSkuBrands = products
                .filter(p => p.sku && p.brand)
                .map(p => ({ sku: p.sku as string, brand: p.brand as string }));
