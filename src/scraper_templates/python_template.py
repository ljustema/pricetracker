...
def scrape(context: Dict[str, Any]):
    ...
    # own_product_sku_brands = context.get('own_product_sku_brands', []) # Add if needed for matching
    ...
    for i, link in enumerate(product_links):
        ...
        try:
            ...
            # if scrape_only_own_products and ean not in own_product_eans:
            #     log_progress(f"Skipping product (not own EAN): {name} ({ean})")
            #     continue
            # Add SKU/Brand matching if needed
