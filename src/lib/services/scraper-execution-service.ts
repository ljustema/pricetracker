...
import ivm from 'isolated-vm'; // Import isolated-vm
...
export class ScraperExecutionService {
  ...
  static async validateScriptSynchronously(
    ...
    let script: ivm.Script | null = null; // Define script variable here

    try {
      addLog('INFO', 'SETUP', 'Creating isolated-vm environment...');
      isolate = new ivm.Isolate({ memoryLimit: MEMORY_LIMIT_MB });
      context = await isolate.createContext();
      const jail = context.global;

      // Set a global property `global` pointing to `jail` (required by some libs)
      await jail.set('global', jail.derefInto());

      // Inject a safe logger
      await jail.set('_log', new ivm.Reference(addLog));
      await context.eval(`
        global.log = (level, phase, message, data) => {
          _log.applyIgnored(undefined, [level, phase, message, data]);
        };
      `);

      // Inject a basic, restricted fetch (consider more robust implementation later)
      // For now, just log the attempt
      await context.eval(`
        global.fetch = async (url, options) => {
          log('INFO', 'FETCH', 'Attempting fetch (validation mode - currently logged only)', { url, options });
          // In a real scenario, use a secure fetch implementation here
          // For validation, we might return mock data or perform a real but restricted fetch
          // Returning a dummy response for now to allow scripts to proceed
          return {
            ok: true,
            status: 200,
      ...
      `);
       ...
       // Inject cheerio if needed by templates (assuming it's installed)
       // Note: This requires cheerio to be bundled or accessible. For simplicity,
       // we might need to pass the cheerio library code into the isolate if it's not globally available.
       // This example assumes a simple case or pre-bundling.
       try {
         const cheerio = await import('cheerio'); // Try importing host cheerio
         // A more robust way would be to read the cheerio bundle and eval it in the isolate
         await jail.set('cheerio', new ivm.Reference(cheerio));
         await context.eval('global.cheerio = cheerio;'); // Expose cheerio
         addLog('INFO', 'SETUP', 'Cheerio library injected (basic).');
       } catch (cheerioError) {
         addLog('WARN', 'SETUP', 'Could not inject host cheerio library. Scripts relying on it might fail.', { error: cheerioError instanceof Error ? cheerioError.message : String(cheerioError) });
       }


      addLog('INFO', 'SETUP', 'Compiling script...');
      script = await isolate.compileScript(scriptContent, { filename: 'scraper.ts' }); // Assign to script variable

      addLog('INFO', 'SETUP', 'Running script in isolate...');
      await script.run(context, { timeout: TIMEOUT_MS });

      // --- Call Script Functions ---
      // Define context object to pass to script functions
      const scriptContext = {
        // Add relevant context here, e.g., from request body or user session
        isTestRun: true, // Validation runs simulate a test run
        isValidationRun: true,
        // filterByActiveBrands: body.filter_by_active_brands ?? false, // Example
        // scrapeOnlyOwnProducts: body.scrape_only_own_products ?? false, // Example
        // activeBrandNames: [], // Fetch if needed
        // ownProductEans: [], // Fetch if needed
        // ownProductSkuBrands: [], // Fetch if needed
      };
      const contextRef = new ivm.Reference(scriptContext);
      await jail.set('_scriptContext', contextRef);


      // 1. Call getMetadata (Optional but good practice)
      try {
        addLog('INFO', 'METADATA', 'Calling getMetadata()...');
        const getMetadataRef = await context.global.get('getMetadata');
        if (typeof getMetadataRef?.apply === 'function') {
           const metadataResult = await getMetadataRef.apply(undefined, [], { timeout: 1000, result: { promise: true, copy: true } });
           addLog('INFO', 'METADATA', 'getMetadata() successful.', { metadata: metadataResult });
        } else {
           addLog('WARN', 'METADATA', 'getMetadata() function not found or not callable in script.');
        }
      } catch (metaError) {
        const errorMsg = metaError instanceof Error ? metaError.message : String(metaError);
        addLog('ERROR', 'METADATA', `Error calling getMetadata(): ${errorMsg}`);
        // Decide if this is a fatal error for validation
      }
      ...
      try {
        addLog('INFO', 'URL_COLLECTION', `Calling collectUrls() with limit ${MAX_URLS}...`);
        const collectUrlsRef = await context.global.get('collectUrls');
        if (typeof collectUrlsRef?.apply !== 'function') {
          throw new Error('collectUrls function is not defined or not callable in the script.');
        }
        // Pass context to the function
        const urlsResult = await collectUrlsRef.apply(undefined, [contextRef], { timeout: TIMEOUT_MS / 2, result: { promise: true, copy: true } });

        if (!Array.isArray(urlsResult)) {
            throw new Error('collectUrls did not return an array.');
        }
        collectedUrls = urlsResult.slice(0, MAX_URLS); // Limit URLs
        addLog('INFO', 'URL_COLLECTION', `collectUrls() successful. Collected ${collectedUrls.length} URLs (limited to ${MAX_URLS}).`, { urls: collectedUrls });

      } catch (urlError) {
        const errorMsg = urlError instanceof Error ? urlError.message : String(urlError);
        addLog('ERROR', 'URL_COLLECTION', `Error during URL collection: ${errorMsg}`);
        // Don't proceed to scraping if URL collection failed fundamentally
         return { valid: false, error: `URL Collection Failed: ${errorMsg}`, logs, products: [] };
      }

      // 3. Call scrapeBatch (only if URLs were collected)
      if (collectedUrls.length > 0) {
        try {
          addLog('INFO', 'BATCH_PROCESSING', `Calling scrapeBatch() for the first batch (${collectedUrls.length} URLs)...`);
          const scrapeBatchRef = await context.global.get('scrapeBatch');
           if (typeof scrapeBatchRef?.apply !== 'function') {
             throw new Error('scrapeBatch function is not defined or not callable in the script.');
           }

          // scrapeBatch should be an async generator
          // We need to call it and iterate through its results within the isolate
          const generatorRef = await scrapeBatchRef.apply(undefined, [new ivm.Reference(collectedUrls), contextRef], { result: { reference: true } });

          // Helper function in isolate to iterate the generator
          await context.eval(`
            async function processGenerator(generatorRef) {
              const results = [];
              const MAX_PRODUCTS_VALIDATION = ${MAX_PRODUCTS};
              try {
                const generator = generatorRef.deref();
                for await (const batchResult of generator) {
                   // Assuming batchResult is an array of products for the batch
                   if (Array.isArray(batchResult)) {
                      results.push(...batchResult);
                      log('DEBUG', 'BATCH_PROCESSING', 'Received batch from generator.', { count: batchResult.length });
                   } else {
                      log('WARN', 'BATCH_PROCESSING', 'Generator yielded non-array value.', { value: batchResult });
                   }
                   if (results.length >= MAX_PRODUCTS_VALIDATION) {
                      log('INFO', 'BATCH_PROCESSING', 'Reached validation product limit, stopping generator.');
                      break; // Stop after reaching the limit
                   }
                }
                log('INFO', 'BATCH_PROCESSING', 'Generator finished.');
                return results.slice(0, MAX_PRODUCTS_VALIDATION); // Ensure limit again
              } catch(genError) {
                 log('ERROR', 'BATCH_PROCESSING', 'Error iterating generator: ' + (genError?.message || genError));
                 throw genError; // Re-throw to be caught outside
              } finally {
                 // Release reference if needed, though it might be automatic on return
                 // generatorRef.release();
              }
            }
          `);

          // Call the helper function
          const processGeneratorRef = await context.global.get('processGenerator');
          const productResults = await processGeneratorRef.apply(undefined, [generatorRef], { timeout: TIMEOUT_MS / 2, result: { promise: true, copy: true } });

          if (Array.isArray(productResults)) {
             products.push(...productResults); // Add validated products
             addLog('INFO', 'BATCH_PROCESSING', `scrapeBatch() processing successful. Found ${products.length} products (limited to ${MAX_PRODUCTS}).`);
          } else {
             addLog('WARN', 'BATCH_PROCESSING', 'scrapeBatch() processing did not return a valid array.', { result: productResults });
          }

        } catch (scrapeError) {
          const errorMsg = scrapeError instanceof Error ? scrapeError.message : String(scrapeError);
          addLog('ERROR', 'BATCH_PROCESSING', `Error during scraping batch: ${errorMsg}`);
          // Allow returning partial results/logs even if scraping fails
        }
      } else {
         addLog('INFO', 'BATCH_PROCESSING', 'Skipping scrapeBatch() as no URLs were collected.');
      }

      addLog('INFO', 'COMPLETION', 'Validation script execution finished.');
      return { valid: true, error: null, products, logs };

    } catch (err) {
      const errorMsg = err instanceof Error ? err.message : String(err);
      const stack = err instanceof Error ? err.stack : undefined;
      addLog('ERROR', 'EXECUTION', `Unhandled error during script validation: ${errorMsg}`, { stack });
      return { valid: false, error: `Script Execution Failed: ${errorMsg}`, logs, products };
    } finally {
      try {
        if (script) script.release(); // Remove isDisposed check
        if (context) context.release(); // Remove isDisposed check
        if (isolate && !isolate.isDisposed) isolate.dispose(); // Keep isDisposed check for isolate
        addLog('INFO', 'CLEANUP', 'isolated-vm environment disposed.');
      } catch (disposeError) {
        addLog('ERROR', 'CLEANUP', `Error disposing isolated-vm environment: ${disposeError instanceof Error ? disposeError.message : String(disposeError)}`);
      }
    }
  }
}