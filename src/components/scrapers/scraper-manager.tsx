...
const PythonScraperForm = dynamic(() => import('./python-scraper-form'), {
  loading: () => <div>Loading...</div>,
  ssr: false
});
...
type Step = 'list' | 'select-type' | 'create-ai' | 'create-python' | 'test' | 'view-logs';
...
export default function ScraperManager({
  ...
  const handleScraperTypeSelect = (type: 'ai' | 'python') => {
    switch (type) {
      case 'ai':
        setStep('create-ai');
        break;
      ...
      case 'view-logs':
        ...
        return (
          <div className="space-y-4">
            ...
            <ScraperLogViewer scraperId={logScraper.id!} />
          </div>
        );
      default:
