...
interface BrandStatisticsProps {
  brands: Brand[];
  onFilterByProductCount?: (min: number, max: number | null) => void;
}

const BrandStatistics: React.FC<BrandStatisticsProps> = ({ brands, onFilterByProductCount }) => {
  const [topCompetitors, setTopCompetitors] = useState<CompetitorStat[]>([]);
  ...
  useEffect(() => {
    const fetchTopCompetitors = async () => {
      try {
        ...
        // Sort competitors by total products and get top 5
        const sortedCompetitors = [...data.competitors]
          .sort((a, b) => b.totalProducts - a.totalProducts)
          .slice(0, 5);

        setTopCompetitors(sortedCompetitors);
      } catch (err) {
  ...
  const handleFilterClick = (min: number, max: number | null) => {
    if (onFilterByProductCount) {
      onFilterByProductCount(min, max);
    }
  };
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
      ...
      <div className="bg-white p-4 rounded-lg shadow border border-gray-200 hover:shadow-md transition-shadow">
        ...
        {isLoading ? (
          <div className="mt-4 text-center text-sm text-gray-500">Loading competitor data...</div>
        ) : error ? (
      ...
      <div className="bg-white p-4 rounded-lg shadow border border-gray-200 hover:shadow-md transition-shadow">
        ...
        <div className="mt-4 space-y-1 text-sm">
          <div
            ...
            className={`flex justify-between cursor-pointer hover:bg-indigo-50 p-1 rounded transition-colors ${onFilterByProductCount && 'hover:bg-indigo-100'}`}
          >
